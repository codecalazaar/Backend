...
//middleware:
const { checkforAunthentication, restrictTo }=require('./middleware/auth')
...
app.use('/',staticRouter);  //get('/') method for all shortids, and  get('/signup') for signup page
           //for each user to have seperate url history 
//--------------------------------------------------
app.use('/url',restrictTo(["NORMAL",'ADMIN']),urlRouter);  //post url to create shortid in database url model
